...
export default function TallyPurchaseVoucher() {
  ...
  const getCurrentDateTime = () => {
    ...
    const time = now.toTimeString().slice(0, 5); // Get HH:MM
    return { date, time };
  };
  ...
  const selectProduct = (rowIndex: number, product: Product) => {
    ...
    // Set quantity to 1 by default when selecting a product
    updateLine(rowIndex, "qty", 1);
    ...
    <Layout>
      <div className="min-h-screen p-6">
        <div className="max-w-6xl mx-auto">
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-2xl font-bold mb-2">Sale Voucher</h1>
            <p className="text-muted-foreground">Add new voucher</p>
          </div>
            ...
            <CardContent className="p-8">
              ...
              <div className="grid grid-cols-3 gap-6 mb-8">
                <div>
                  ...
                  <Input
                    value={voucher.voucherNumber || "SAL-2024-0001"}
                    readOnly
                ...
                <div>
                  ...
                  <Input
                    type="date"
                    value={voucher.date.split("T")[0]}
                    onChange={(e) => {
                      const time = new Date(voucher.date)
                        .toTimeString()
                        .slice(0, 5);
                      setVoucher({
                        ...voucher,
                ...
                <div>
                  ...
                  <Input
                    type="time"
                    value={new Date(voucher.date).toTimeString().slice(0, 5)}
                    onChange={(e) => {
                      const [hours, minutes] = e.target.value.split(":");
                      const dateStr = voucher.date.split("T")[0];
                      const newDate = new Date(
                        `${dateStr}T${e.target.value}:00`
                      );
                      setVoucher({ ...voucher, date: newDate.toISOString() });
                    }}
                    className="border-border"
                  />
                </div>
            ...
            <CardContent className="p-8">
                      ...
                      <table className="w-full">
                        <thead className="bg-muted/50">
                          <tr>
                            ...
                            <th className="px-3 py-4 text-center text-xs font-semibold uppercase tracking-wider text-muted-foreground w-32">
                              Available Stock
                            </th>
                              ...
                              <td className="px-3 py-4 text-center">
                                <Input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (
                                      selectedProduct &&
                                      newQty > selectedProduct.availableStock
                                    ) {
                                      alert(
                                        `Only ${selectedProduct.availableStock} ${selectedProduct.uom} available in stock`
                                      );
                                      return;
                                    }
                                    updateLine(index, "qty", newQty);
                                  }}
                                  onFocus={() =>
                                    setFocusedCell({ row: index, col: "qty" })
                                  }
                                  placeholder="0"
                                  className="w-20 text-center border-0 focus-visible:ring-0 focus-visible:ring-offset-0"
                                />
                              </td>
                              <td className="px-3 py-4 text-center">
                                {line.productId ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                  ...
                  <select
                    className="mt-2 w-full p-3 rounded-md bg-background border border-border"
                    defaultValue="CASH"
                  >
